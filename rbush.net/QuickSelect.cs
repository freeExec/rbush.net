...
namespace rbush.net
{
    ...
    {
        public static T Select(IList<T> list, int indexK)
        {
            return Select(list, indexK, 0, list.Count);
        }

        public static T Select(IList<T> list, int indexK, int left, int right)
        {
            while (right > left)
            {
                if (right - left > 600)
                {
                    ...
                    var newLeft = (int)Math.Max(left, Math.Floor(indexK - m * s / n + sd));
                    var newRight = (int)Math.Min(right, Math.Floor(indexK + (n - m) * s / n + sd));
                    Select(list, indexK, newLeft, newRight);
                }
                ...
                Swap(list, left, indexK);
                if (list[right].CompareTo(t) > 0) Swap(list, left, right);
                ...
                while (i < j)
                {
                    ...
                    i++;
                    j--;
                    while (list[i].CompareTo(t) < 0) i++;
                    while (list[j].CompareTo(t) > 0) j--;
                }

                if (list[left].CompareTo(t) == 0) Swap(list, left, j);
                else
                {
                    j++;
                    Swap(list, j, right);
